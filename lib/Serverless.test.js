...
describe('Serverless [new tests]', () => {
  describe('When local version available', () => {
    before(() => {
      const fixturePath = path.resolve(fixtures.map.aws, 'node_modules/serverless');
      const fixtureModulePath = path.resolve(fixturePath, 'index.js');
      return fse
        .copy(path.resolve(fixtures.map.aws, 'node_modules-fixture/serverless'), fixturePath)
        .then(() => fse.readFile(fixtureModulePath))
        .then(content =>
          fse.writeFile(
            fixtureModulePath,
            String(content).replace('$SERVERLESS_PATH', path.resolve(__dirname, '../'))
          )
        );
    });
    ...
    describe('When running local version', () => {
      it('Should run without notice', () =>
        runServerless({
          serverlessPath: path.resolve(fixtures.map.aws, 'node_modules/serverless'),
          cwd: fixtures.map.aws,
          cliArgs: ['-v'],
        }).then(({ serverless }) => {
          expect(Array.from(serverless.triggeredDeprecations)).to.deep.equal([]);
          expect(serverless.invokedInstance).to.not.exist;
