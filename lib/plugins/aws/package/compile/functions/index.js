...
class AwsCompileFunctions {
    ...
    return BbPromise.try(() => {
      ...
      if (functionObject.onError) {
        ...
        if (typeof arn === 'string') {
          ...
          functionObject.iamConfig.policyStatements.push({
            Effect: 'Allow',
            Action: ['sns:Publish'],
            Resource: [arn],
          });
      ...
      if (tracing) {
        ...
        // update the PolicyDocument statements
        functionObject.iamConfig.policyStatements.push(stmt);
      }
  ...
  compileFunctionEventInvokeConfig(functionName) {
    const functionObject = this.serverless.service.getFunction(functionName);
    const { destinations, maximumEventAge, maximumRetryAttempts, iamConfig } = functionObject;
    ...
    if (destinations) {
      ...
      if (destinations.onSuccess) {
        if (!hasAccessPoliciesHandledExternally) {
          this.ensureTargetExecutionPermission(destinations.onSuccess, iamConfig);
        }
      ...
      if (destinations.onFailure) {
        if (!hasAccessPoliciesHandledExternally) {
          this.ensureTargetExecutionPermission(destinations.onFailure, iamConfig);
        }
        destinationConfig.OnFailure = {
          Destination: destinations.onFailure.startsWith('arn:')
            ? destinations.onFailure
            : this.provider.resolveFunctionArn(destinations.onFailure, iamConfig),
        };
      }
    }
  ...
  ensureTargetExecutionPermission(functionAddress, iamConfig) {
    const action = (() => {
    ...
    iamConfig.policyStatements.push({
      Effect: 'Allow',
      Action: action,
